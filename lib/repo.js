...
const toSettings = (row) => ({
  ...
  fundTarget: row?.fund_target_agorot ?? 0,
});
...
const toItem = (row) => ({
  ...
  owners: (row.item_owners ?? []).map((link) => link.user_id),
  note: row.note,
...
export async function updateSettings(patch) {
  ...
  if ('fundTarget' in patch) row.fund_target_agorot = patch.fundTarget;
...
export async function listItems() {
  const { data, error } = await db().from('items').select('*, item_domains(domain), item_owners(user_id)');
...
export async function createItem(fields, userId) {
  ...
  if (fields.owners) await setItemOwners(data.id, fields.owners);
  return { ...toItem(data), domains: fields.domains ?? [], owners: fields.owners ?? [] };
}

/** Replace a task's owners wholesale, for the same reason domains are. */
export async function setItemOwners(itemId, owners) {
  const client = db();
  const { error: cleared } = await client.from('item_owners').delete().eq('item_id', itemId);
  fail(cleared, 'clear owners');

  const wanted = [...new Set(owners)].filter(Boolean);
  if (wanted.length === 0) return;

  const { error } = await client
    .from('item_owners')
    .insert(wanted.map((userId) => ({ item_id: itemId, user_id: userId })));
  fail(error, 'set owners');
}
...
export async function updateItem(id, patch, userId) {
  ...
  const { data, error } = await db()
    .from('items').update(row).eq('id', id).select('*, item_domains(domain), item_owners(user_id)').single();
  fail(error, 'update item');

  const result = toItem(data);
  if (patch.domains) {
    await setItemDomains(id, patch.domains);
    result.domains = patch.domains;
  }
  if (patch.owners) {
    await setItemOwners(id, patch.owners);
    result.owners = patch.owners;
  }
  return result;
}
...
export async function createSubtask(itemId, title, userId, parentId = null) {
  const { data, error } = await db().from('subtasks')
    .insert({ item_id: itemId, title, created_by: userId, parent_id: parentId || null })
    .select().single();
  fail(error, 'add step');
...
export async function listDeposits() {
  const { data, error } = await db()
    .from('fund_deposits').select('*').order('deposited_on', { ascending: false });
  fail(error, 'list deposits');
  return (data ?? []).map(toDeposit);
}

export async function createDeposit(fields, userId) {
  const { data, error } = await db().from('fund_deposits').insert({
    user_id: fields.userId,
    amount_agorot: fields.amount,
    deposited_on: fields.depositedOn,
    note: fields.note || null,
    created_by: userId,
  }).select().single();
  fail(error, 'record deposit');
  return toDeposit(data);
}

export async function deleteDeposit(id) {
  const { error } = await db().from('fund_deposits').delete().eq('id', id);
  fail(error, 'delete deposit');
}
...
/** A line about how the step actually went. Empty clears it rather than storing
 *  a blank, so "nothing written" is one state and not two. */
export async function setSubtaskConclusion(id, conclusion) {
  const text = typeof conclusion === 'string' ? conclusion.trim() : '';
  const { data, error } = await db().from('subtasks')
    .update({ conclusion: text || null }).eq('id', id).select().single();
  fail(error, 'write step conclusion');
  return toSubtask(data);
}
...
/**
 * Move a step: to a new parent, a new place among its siblings, or both.
 *
 * Whether the move is legal is decided by core against the task's current rows,
 * not here and not by the client. The client can only say where a finger was
 * lifted; a request that arrived while somebody else was rearranging the same
 * task has to be judged against what is in the table now.
 *
 * Positions are written one row at a time rather than in a transaction. That is
 * honest about what this is: a half-applied reorder leaves the steps in a
 * different order than intended, which the next drag fixes, and which nobody
 * loses anything over. The move that *would* lose something — a step into its
 * own subtree — is refused before a single write goes out.
 */
export async function moveSubtask(id, parentId, index) {
  const rows = await listSubtasks();
  const moving = rows.find((row) => row.id === id);
  if (!moving) throw new Error('That step no longer exists.');

  const siblings = rows.filter((row) => row.itemId === moving.itemId);
  const problem = moveProblem(id, parentId ?? null, siblings);
  if (problem) throw new Error(problem);

  for (const write of placeSubtask(siblings, id, parentId ?? null, index)) {
    const patch = { position: write.position };
    if ('parentId' in write) patch.parent_id = write.parentId;

    const { error } = await db().from('subtasks').update(patch).eq('id', write.id);
    fail(error, 'move step');
  }
}
...
export async function claimLock(id, userId, now = Date.now()) {
  ...
  const { data, error } = await db().from('items')
    ...
    .select('*, item_domains(domain), item_owners(user_id)')
    .maybeSingle();
...
export async function loadEverything(now = new Date()) {
  const [settings, items, payments, users, domains, subtasks, deposits] = await Promise.all([
  ...
  return { today, settings, users, domains, items, subtasks, payments, deposits, spend };
}
