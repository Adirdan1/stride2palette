 ...
 *
 * Middleware has already done the HMAC check, so routes must not repeat it. They
 * read this header instead — which makes it a trusted input, and the one thing
 * that must never happen is a client setting it themselves. Middleware strips it
 * from every incoming request, unconditionally, before anything else runs.
 */
export const USER_HEADER = 'x-palette-user';

/**
 * A year, matching Stride. Adir's call, made deliberately.
 *
 * This was thirty days first, on the reasoning that Stride is one person's phone
 * while this app holds a business's finances and has staff in it — so the worst
 * case is not someone staying signed in, it is a device that left with somebody.
 *
 * A year is the answer anyway, because being asked for a PIN is friction that
 * lands on people who are trying to do their jobs, and an expiry is a blunt
 * instrument for a problem that has a sharp one: the `sessions` table. Deleting
 * a row revokes that device immediately, which is both faster and more precise
 * than any expiry, whenever a phone is lost or somebody leaves.
 *
 * The cost is that revoking is a thing someone has to remember to do, where an
 * expiry happens on its own. If that ever proves optimistic, this is one line.
 */
 ...
 *
 * Both sides are hashed first so the comparison runs over equal-length strings
 * whatever was presented, which is what keeps the length of the real secret from
 * leaking through the early exit in `timingSafeEqual`.
 */
export async function matchesSecret(presented, expected) {
  if (!expected || typeof presented !== 'string') return false;

  // Both sides are trimmed. Surrounding whitespace in a setup key is never
  // intentional: the configured value is pasted into a dashboard field and the
  // presented one is typed or pasted on a phone, and both routinely pick up a
  // trailing newline or space. Refusing a key that is right apart from
  // invisible characters teaches nothing and wastes an evening — this app's
  // first deploy lost one to exactly that, in the Supabase key.
  //
  // This does not weaken the comparison. Trimming cannot make two different
  // secrets equal, and the hash-then-compare below is unchanged.
  const [a, b] = await Promise.all([
    hashSessionToken(presented.trim()),
    hashSessionToken(String(expected).trim()),
  ]);
