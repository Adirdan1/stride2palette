...
/**
 * Planned versus actual, overall and by category.
 *
...
export function summariseBudget(items, payments) {
  ...
  const bucket = (category) => {
    if (!byCategory.has(category)) byCategory.set(category, { category, planned: 0, actual: 0 });
  ...
  for (const item of items) {
    categoryOf.set(item.id, item.category);
    const row = bucket(item.category);
    if (item.status !== 'dropped') {
      planned += item.planned ?? 0;
      row.planned += item.planned ?? 0;
    }
  }

  let actual = 0;
  for (const payment of payments) {
    actual += payment.gross;
    bucket(categoryOf.get(payment.itemId) ?? 'other').actual += payment.gross;
  }
  ...
  return {
    ...
    byCategory: [...byCategory.values()].sort((a, b) => b.actual - a.actual || b.planned - a.planned),
  };
}
